...
export const ScrollbarIndicator = ({active, scrollIndex, sectionHeights}) => {
  ...
  const animatedTranslateStyles = useAnimatedStyle(() => {
    return {
      transform: [{translateY: scrollIndex.value * sectionHeights.scrollbar}],
    };
  });
  ...
  return (
    <Animated.View
      style={[
        styles(sectionHeights.scrollbar).scrollbarControllerView,
        animatedTranslateStyles,
      ]}>
      <Animated.View
        style={[
          styles(sectionHeights.scrollbar).scrollbarController,
          animatedStyles,
        ]}
      />
    </Animated.View>
...
const styles = scrollbarSectionHeight =>
  StyleSheet.create({
    scrollbarController: {
